...
typedef struct
{
    ...
    /*
     * If dma_audio_channels is populated with a non-zero value, the same count will be used
     * for transmit and receive.  However, if dma_audio_channels is zero, the
     * dma_audio_tx_channels and dma_audio_rx_channels fields will be used.  This allows for
     * DMAs that have a different number of transmit and receive channels.
     */
    uint16_t dma_audio_channels, dma_audio_tx_channels, dma_audio_rx_channels;

    // Number of audio samples per block / frame
    uint16_t dma_audio_block_size;

    // Whether the secondary data pin should be used
    bool use_secondary_data_pin;

	#pragma alignment_region(32)
    // DMA descriptors for DMA ping-pong
